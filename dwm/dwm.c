...
enum { SchemeNorm, SchemeSel,
       ...
       SchemeTag4, SchemeTag5, SchemeTag6, SchemeLayout }; /* color schemes */
...
/*------------------------------------------------*/
		...
		if (ev->x > selmon->ww - TEXTW(stext))			click = ClkStatusText;
		}
...
stairs(Monitor *m)
{
	...
	for (i = 0, my = m->gappih, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			h = (m->wh - my) / (MIN(n, m->nmaster) - i) - m->gappih;
			resize(c, m->wx + m->gappih, m->wy + my, mw - (2 * c->bw) - m->gappih, h - (2 * c->bw), 0);
			...
			resize(c,
			       m->wx + mw + (ox * stairpx) + m->gappih,
			       m->wy + (oy * stairpx) + m->gappih,
			       m->ww - mw - (2 * c->bw) - (ow * stairpx) - (2 * m->gappih),
			       m->wh - (2 * c->bw) - (oh * stairpx) - (2 * m->gappih),
			       0);
		}
	}
}
