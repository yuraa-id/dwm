...
if [ $wlan_online ]; then
    ...
    printf "^c$white^ $internet_logo ^c$green_alt^ ^c$grey_alt^%3s ^c$green_alt^ ^c$grey_alt^%3s - ^c$green^Connected\\n" \
...
clock() {
	printf "^c$white^^b$dwm_alt^📅"
	printf "^c$white^^b$dwm^ $(date "+%b %d | %I:%M%p")"
}
...
while true; do
  ...
  sleep 1 && xsetroot -name "$(separator) $(wlan_net) $(volume) $(brightness) $(get_cputemp) $(battery_status) $(battery_capacity) $(clock) $(separator)"
done
